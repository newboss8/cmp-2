...
$editor_password = "";
//可编辑文件地址列表
$files = array("../config.xml", "../list.xml", "../config_p.xml", "../list_p.xml",);
//
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    <?php
    if (empty($_GET["id"]) || !is_numeric($_GET["id"]) || $_GET["id"] >= sizeof($files)) {
      $file_id = 0;
    } else {
      $file_id = $_GET["id"];
    }
    ...
    if (!empty($_POST["cmd"])) {
      ...
      if ($cmd == "login") {
        ...
        if ($submit_password == $editor_password) {
          if ($editor_password == "") {
            echo '<div class="error_msg">你未设置密码！请打开cmp\p\editor.php，修改 $editor_password = ""; 里的引号内为你自己的密码。</div>';
            exit;
          } else {
  ...
  for ($i = 0; $i < $num; $i++) {
    ...
    ?>
            <label for="file<?php echo $i ?>">
              <input type="radio" <?php echo $checked ?> id="file<?php echo $i ?>" onclick="this.form.submit();" name="id" value="<?php echo $i ?>" />
    <?php echo $files[$i] ?></label>
              <?php
            }
            ?>
        </form>
      ...
      <div class="file_edit">
        <form action="<?php //echo $path; ?>" method="post" onsubmit="return check(this);">
  <?php
  $file_content = file_read($files[$file_id]);
  ?>
          <textarea name="file_content" class="file_content"><?php echo $file_content ?></textarea>
